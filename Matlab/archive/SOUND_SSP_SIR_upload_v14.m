...
for maincounter=1:length(all_datasets_list) %loops through all datasets  %loops through all datasets
    ...
    % Use TESA's built-in SOUND implementation instead of custom SOUND_alt
    fprintf('\n--- Starting SOUND Algorithm ---\n');
    
    % SOUND algorithm parameters (as per original code)
    lambda_value = 0.1;  % Regularization parameter
    iter = 5;            % Number of iterations
    
    % Try TESA implementation first, then fallback to custom implementation
    try
        % Check if pop_tesa_sound is available
        if exist('pop_tesa_sound', 'file') == 2
            fprintf('Using TESA SOUND implementation...\n');
            EEG = pop_tesa_sound(EEG, 'lambdaValue', lambda_value, 'iter', iter, ...
                                'leadfieldInFile', [], 'leadfieldChansFile', [], ...
                                'replaceChans', [], 'multipleConds', []);
            EEG_clean = EEG;
            
        else
            % Fallback: Use custom SOUND implementation
            fprintf('TESA SOUND not available. Using custom SOUND implementation...\n');
            
            % Add the current directory to path to find SOUND_alt and DDWiener
            current_dir = fileparts(mfilename('fullpath'));
            addpath(current_dir);
            
            % Check if custom functions exist
            if exist('SOUND_alt', 'file') == 2 && exist('DDWiener', 'file') == 2
                fprintf('Using custom SOUND_alt and DDWiener functions...\n');
                ...
                for SOUND_maincounter = 1:size(EEG.data,2)
                    if mod(SOUND_maincounter, 1000) == 0
                        fprintf('Processing time sample %d/%d\n', SOUND_maincounter, size(EEG.data,2));
                    end
                    ...
                    [~, ~, sigmas] = SOUND_alt(tmp_data, LFM_sphere, iter, lambda_value, sigmas, 0);
                    
                    all_sigmas(:,SOUND_maincounter) = sigmas;
                end
                
                % Apply temporal smoothing and final cleaning
                ...
                % Create smoothing function (skip plot for automated processing)
                smoothing_func = exp(-0.5*((1:(2*scaling_w + 1)) - (scaling_w + 1)).^2/smoothness^2);
                ...
                % Neither TESA nor custom functions available
                fprintf('No SOUND implementation available. Skipping SOUND step.\n');
                EEG_clean = EEG;
            end
        end
        
    catch ME
        % If any SOUND implementation fails, continue without it
        fprintf('SOUND algorithm failed: %s\n', ME.message);
        fprintf('Continuing without SOUND cleaning.\n');
        EEG_clean = EEG;
    end
